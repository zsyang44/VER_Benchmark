...
class ToTensor(SpatialTransform):
    ...
    def __call__(self, pic):
        ...
        if pic.mode != "RGB":
            ...
            # img = torch.ByteTensor(torch.ByterStorage.from_buffer(pic.tobytes()))
            img = pic.tobytes()
...
class Scale(SpatialTransform):
    ...
    def __init__(self, size, interpolation=Image.BILINEAR):
        ...
        int, smaller edge of the image will be matched to this number, i.e. if height > width, then the image will be
...
class RandomChoice(SpatialTransform):
    ...
    def __call__(self, img):
        return self.transform_to_apply(img)

    def randomize_parameters(self, recursive=True):
        self.transform_to_apply = self.transforms[random.randint(0, len(self.transforms) - 1)]
        if recursive:
            self.transform_to_apply.randomize_parameters()
